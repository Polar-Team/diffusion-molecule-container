...
while [ "${i}" -le "${max_attempts}" ]; do
  # Safely extract indexed environment variables
  git_user=$(printenv "GIT_USER_${i}" 2>/dev/null || echo "")
  git_password=$(printenv "GIT_PASSWORD_${i}" 2>/dev/null || echo "")
  git_url=$(printenv "GIT_URL_${i}" 2>/dev/null || echo "")

  # Skip if all three are empty (end of sequence)
  if [ -z "${git_user}" ] && [ -z "${git_password}" ] && [ -z "${git_url}" ]; then
    break
  fi

  # Validate all three are present before configuring
  if [ -n "${git_user}" ] && [ -n "${git_password}" ] && [ -n "${git_url}" ]; then
    echo "Configuring git credentials for ${git_url} (index ${i})"
    _configure_git "${git_user}" "${git_password}" "${git_url}"
  else
    echo "Warning: Incomplete git configuration at index ${i} (skipping)" >&2
  fi
